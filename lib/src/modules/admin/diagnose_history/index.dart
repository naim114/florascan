...
import '../../diagnose/history_view.dart';

class AdminPanelDiagnoseHistory extends StatefulWidget {
  final List<DiagnoseHistoryModel> diagnoseList;
  final Function(bool refresh) notifyRefresh;

  const AdminPanelDiagnoseHistory({
    super.key,
  ...
  @override
  State<AdminPanelDiagnoseHistory> createState() =>
      AdminPanel_DiagnoseHistoryState();
}

class AdminPanel_DiagnoseHistoryState extends State<AdminPanelDiagnoseHistory> {
                      ...
                      onSort: (columnIndex, _) {
                        setState(() {
                          ...
                          if (_isAscending == true) {
                            _isAscending = false;
                            widget.diagnoseList.sort((itemA, itemB) =>
                                itemA.user!.name.compareTo(itemB.user!.name));
                          } else {
                            _isAscending = true;
                            widget.diagnoseList.sort((itemA, itemB) =>
                                itemB.user!.name.compareTo(itemA.user!.name));
                          }
                        });
                      },
                    ),
